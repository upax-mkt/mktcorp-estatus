 ...
 * consultas que hoy hace `sesionesPublicasDelMes` por la TABLA de la que
 ...
 * (la de `sesiones` con su JOIN) sigue la cadena completa hasta `orderBy` y
 * resuelve con `filasSesiones`.
 *
...
let filasSesiones: { salaSlug: string | null; fecha: Date; salaNombre: string | null; salaColor: string | null }[] = []
...
// SESIONES, no el registro de salas (eso ya lo cubre src/db/temas.test.ts).
    ...
    select: () => ({
      from: (tabla: unknown) => {
        ...
        const encadenable = {
          ...
          where: () => encadenable,
          orderBy: () => Promise.resolve(filasSesiones),
        }
        return encadenable
      },
    }),
...
describe('sesionesPublicasDelMes — grupo-upax nunca sale, aunque tenga tema válido', () => {
  ...
  it('si TODAS las filas del mes son de grupo-upax, devuelve la lista vacía, no un error', async () => {
    filasSesiones = [{ salaSlug: 'grupo-upax', fecha: new Date('2026-08-05T16:00:00.000Z'), salaNombre: 'Grupo UPAX', salaColor: '#E34714' }]
  ...
  it('una UDN real sigue resolviendo su nombre y color, tal cual los trae la fila', async () => {
    filasSesiones = [{ salaSlug: 'neracode', fecha: new Date('2026-08-05T16:00:00.000Z'), salaNombre: 'NeraCode', salaColor: '#3E31CC' }]
