...
import {
  salaMasDesatendida, estatusVigente, estatusEfectivo, estaCongelado,
  ordenarPorProximaReunion, acuerdosVencidos, acuerdosAbiertos, temperatura, type EstadoSala,
} from './salas'

// `sesionesSinMinuta` vivió aquí hasta la Tarea 7: leía `EstadoSala.presentaciones`/
// `.minutas`, retirados con esa tarea (`EstadoSala.reuniones` los sustituye —
// ver dominio/reunion.ts), y ya no tenía más llamador que su propio test
// (comprobado con grep antes de retirarla). "De qué reuniones falta minuta"
// hoy se lee directo de `Reunion.minuta` en cada elemento de
// `EstadoSala.reuniones`, sin falta de una función aparte.
...
describe('ordenarPorProximaReunion', () => {
  it('primero las que tienen fecha, de la más próxima a la más lejana', () => {
    const orden = ordenarPorProximaReunion([
      sala({ slug: 'zeus', nombre: 'Zeus', proximaReunion: '2026-08-20' }),
      sala({ slug: 'neracode', nombre: 'NeraCode', proximaReunion: '2026-08-03' }),
      sala({ slug: 'uix', nombre: 'UiX', proximaReunion: '2026-08-11' }),
    ])
  ...
  it('las que no tienen fecha van después, por nombre', () => {
    const orden = ordenarPorProximaReunion([
      sala({ slug: 'uix', nombre: 'UiX', proximaReunion: null }),
      sala({ slug: 'neracode', nombre: 'NeraCode', proximaReunion: null }),
      sala({ slug: 'zeus', nombre: 'Zeus', proximaReunion: '2026-08-20' }),
    ])
  ...
  it('las pausadas van al final, aunque tengan la fecha más próxima de todas', () => {
    const orden = ordenarPorProximaReunion([
      sala({ slug: 'zeus', nombre: 'Zeus', proximaReunion: '2026-08-01', activa: false }),
      sala({ slug: 'neracode', nombre: 'NeraCode', proximaReunion: null }),
      sala({ slug: 'uix', nombre: 'UiX', proximaReunion: '2026-08-11' }),
    ])
